...
func (r *CECResourceParser) ParseResources(cecNamespace string, cecName string, xdsResources []cilium_v2.XDSResource, isL7LB bool, injectCiliumEnvoyFilters bool, useOriginalSourceAddr bool, newResources bool) (envoy.Resources, error) {
	...
	// Check if secrets are referenced but not present as explicit Secret resources
	allReferencedSecretNames := make(map[string]bool)
	for _, listener := range resources.Listeners {
		for _, fc := range listener.FilterChains {
			if fc.TransportSocket != nil {
				if tc := fc.TransportSocket.GetTypedConfig(); tc != nil {
					any, err := tc.UnmarshalNew()
					if err == nil {
						var tlsContext *envoy_config_tls.CommonTlsContext
						switch transport := any.(type) {
						case *envoy_config_tls.DownstreamTlsContext:
							tlsContext = transport.CommonTlsContext
						case *envoy_config_tls.UpstreamTlsContext:
							tlsContext = transport.CommonTlsContext
						case *envoy_config_quic.QuicDownstreamTransport:
							if transport.DownstreamTlsContext != nil {
								tlsContext = transport.DownstreamTlsContext.CommonTlsContext
							}
						case *envoy_config_quic.QuicUpstreamTransport:
							if transport.UpstreamTlsContext != nil {
								tlsContext = transport.UpstreamTlsContext.CommonTlsContext
							}
						}
						if tlsContext != nil {
							for _, sc := range tlsContext.TlsCertificateSdsSecretConfigs {
								allReferencedSecretNames[sc.Name] = true
							}
							if sc := tlsContext.GetValidationContextSdsSecretConfig(); sc != nil {
								allReferencedSecretNames[sc.Name] = true
							}
							if cvc := tlsContext.GetCombinedValidationContext(); cvc != nil {
	...
	if len(allReferencedSecretNames) > 0 && len(resources.Secrets) == 0 {
		referencedSecretNamesList := make([]string, 0, len(allReferencedSecretNames))
		for name := range allReferencedSecretNames {
			referencedSecretNamesList = append(referencedSecretNamesList, name)
		}
		r.logger.Warn("ParseResources: secrets are referenced in listeners but not present as explicit Secret resources",
			logfields.K8sNamespace, cecNamespace,
			logfields.Name, cecName,
			"referencedSecretNames", referencedSecretNamesList,
		)
	}
	...
	if ts != nil {
		if tc := ts.GetTypedConfig(); tc != nil {
			...
			case *envoy_config_quic.QuicDownstreamTransport:
				...
				if tls.DownstreamTlsContext != nil {
